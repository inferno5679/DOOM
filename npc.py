...
class NPC(AnimatedSprite):
    def __init__(self, game, path='game_dev/resources/sprites/npc/soldier/0.png', pos=(10.5,5.5), scale=0.6, shift=0.38, animation_time=180):
    ...
    def animate_death(self):
        if not self.alive:
            if self.game.global_trigger and self.frame_counter < len(self.death_images):
                self.death_images.rotate(-1)
                self.image = self.death_images[-1]
                self.frame_counter +=1

    def check_hit_npc(self):
        if self.ray_cast_value and self.game.player.shot and self.alive:
    ...
    def ray_cast_player_npc(self):
        ...
        for i in range(max_depth):
            ...
            if tile_hor == self.map_pos:
                player_distance_horizontal = depth_hor
                break
            if tile_hor in self.game.map.world_map:
                wall_distance_horizontal = depth_hor
                break
            x_hor += dx
            y_hor += dy
